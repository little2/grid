...
def _decide_layout_by_duration(duration_sec: float) -> dict:
    ...
    if m < 5:
        ...
        return {"cols": 4, "rows": 4, "num_aux": 12, "sample_count": 200}
    elif m < 30:
        return {"cols": 5, "rows": 5, "num_aux": 21, "sample_count": 300}
    else:
        return {"cols": 6, "rows": 6, "num_aux": 32, "sample_count": 400}
...
def make_hero_grid_adaptive(
    ...
    num_aux: int | None = None,               
    manual_times: List[str | float | int] | None = None,
    refine_window: float = 0.1,               # 新增：精修搜索窗口秒数
    refine_step: float = 0.1,                 # 新增：精修步进秒数
    refine_min_face_area: float = 1e-4,       # 新增：精修人脸面积阈值
) -> Dict[str, Any]:
    """
    自适应网格图：
    ...
    with VideoFileClip(video_path, audio=False) as clip:
        ...
        # 获取视频分辨率
        video_w, video_h = clip.size  # 等价于 clip.w, clip.h
        _stage(f"视频原始分辨率宽高：{video_w}x{video_h}")
        ...
        refined_img, refined_meta = _refine_hero_nearby(
            ...
            window=refine_window,           # 改成用外部参数
            step=refine_step,
            min_face_area=refine_min_face_area
        )
    ...
    _stage(f"单格截图尺寸宽x高：{base_w}x{base_h}")  # ← 新增
    ...
    _stage(f"Grid截图尺寸宽x高：{grid_w}x{grid_h}")  # ← 新增

    grid_stretch = grid.resize((cell_h*rows, cell_w*rows), Image.LANCZOS)
    print(f"网格尺寸：{grid_w}x{grid_h} (拉伸后 {grid_h}x{grid_w})")
    grid_stretch.save(out_path, quality=90, optimize=True)
    ...
    try:
        ...
        if Mode == 'quick':
            ...
            meta = make_hero_grid_adaptive(
                video_path="video/video_20200603_214657.mp4",
                preview_basename="previews/390550",
                sample_count=12,  # 主图 + 6 辅助图
                num_aux=6,
                refine_window=0.1,           # 主图附近搜索 ±N 秒
                refine_step=0.1,            # 每 N 秒取一帧
                refine_min_face_area=5e-4    # 最小人脸面积阈值
            )
        else:
            # 完整模式：生成主图 + 辅助图（精修）

...
def _decide_layout_by_duration(duration_sec: float) -> dict:
    ...
    m = duration_sec / 60.0
    if m < 5:
        return {"cols": 3, "rows": 4, "num_aux": 8,  "sample_count": 200}
    elif m < 10:
        return {"cols": 4, "rows": 4, "num_aux": 12, "sample_count": 300}
    elif m < 30:
        return {"cols": 5, "rows": 5, "num_aux": 21, "sample_count": 400}
    else:
        return {"cols": 6, "rows": 6, "num_aux": 32, "sample_count": 500}
...
def make_hero_grid_adaptive(
    ...
    grid_stretch = grid.resize((2160, 3840), Image.LANCZOS)
